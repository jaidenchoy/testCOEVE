...
#' @param .extrapolation An integer that corresponds to sample extrapolation size.
...
#' }
#' @export repDiversity
repDiversity <- function(.data, .method, .max.q = 6, .min.q = 1, .q = 5, .step = NA, .bound = 2,
                         .quantile = c(.025, .975), .extrapolation = 200000, .perc = 50,
                         .verbose = T, .do.norm = NA, .laplace = 0) {
  .method = .method[1]
  if (has_class(.data, 'list') && .method != "raref"  && .method != "rarefaction") {
    res = lapply(.data, repDiversity, .method = .method,
                  .max.q = .max.q, .min.q = .min.q, .q = .q,
                  .step = .step, .bound = .bound, .quantile = .quantile,
    ...
    return(res)
  } else if (.method == "raref" || .method == "rarefaction") {
    if (!has_class(.data, "list")) {
      .data = list(Sample = .data)
    }
    vec = lapply(.data, function (x) collect(select(x, IMMCOL$count), n = Inf)[[1]])
  } else {
    vec <- collect(select(.data, IMMCOL$count), n = Inf)[[1]]
  }
  switch(.method,
         ...
         raref = rarefaction(.data = vec, .step = .step, .quantile = .quantile,
                             .extrapolation = .extrapolation, .verbose = .verbose),
         rarefaction = rarefaction(.data = vec, .step = .step, .quantile = .quantile,
                             .extrapolation = .extrapolation, .verbose = .verbose),
...
rarefaction <- function (.data, .step = NA, .quantile = c(.025, .975),
  ...
  if (is.na(.step)) {
    .step = min(sapply(.data, function (x) sum(x))) %/% 50.
  }
  ...
  muc.list <- lapply(1:length(.data), function (i) {
    ...
    if (.extrapolation > 0) {
      ...
      sizes <- seq(tail(seq(.step, sum(.data[[i]]), .step), 1) + .step, .extrapolation + max(sapply(.data, function (x) sum(x))), .step)
      if (length(sizes) != 1) {
      ...
      data.frame(muc.res, Sample = names(.data)[i], stringsAsFactors = F)
    }
  })
  if (.verbose) close(pb)
