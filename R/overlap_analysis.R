...
#' @param .analysis A string that defines the type of analysis to perform.
...
#'
#' @export repOverlapAnalysis rep.ova
repOverlapAnalysis <- function (.data, .analysis = ("hclust"), .scale = default_scale_fun, .raw=T,
                                .perp = 1, .theta = .1, .eps = .01, .k = 2) {
  if (length(.analysis) > 2) { stop("Can't process the .analysis parameter with length more than 2.") }

  .analysis = strsplit(.analysis, "+", T)[[1]]
  ...
  if (.analysis[1] %in% c("mds", "tsne")) {
    res = switch(.analysis[1],
                 tsne = immunr_tsne(.data, .dist = T, .perp = .perp, .theta = .theta),
  ...
  if (length(.analysis) == 1) {
    ...
    if (.analysis %in% c("hclust", "kmeans", "dbscan", "permut")) {
      if (has_class(res, "immunr_mds")) {
        res = res$x
      }

      res = switch(.analysis,
                   hclust = immunr_hclust(res, .k = .k, .dist = F),
                   kmeans = immunr_kmeans(res, .k = .k),
                   dbscan = immunr_dbscan(res, .eps = .eps, .dist = F),
