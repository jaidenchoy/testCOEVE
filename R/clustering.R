...
#'
#' immunr_kmeans(.data, .k = 2, .k.max = as.integer(sqrt(nrow(.data))) + 1, .nbclust_method = c("silhouette", "gap_stat"))
#'
#' immunr_dbscan(.data, .eps, .dist = T)
#'
#' @param .data Matrix or data frame with features, distance matrix or output from \link{repOverlapAnalysis}, \link{geneUsageAnalysis} or \link{repDiversityAnalysis} functions.
#'
...
#'
#' @seealso \link{vis.immunr_hclust}
